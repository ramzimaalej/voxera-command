...
This repo (`voxera-command`) holds only the **business / CEO** processes — `iterate-doc.js`, `iterate-vision.js`, `weekly-business-review.js`, `revenue-pulse.js` — in `.a5c/processes/`. They follow the same conventions. The engineering processes (`build-feature`, `fix-bug`, `design-review`) live in `voxera-os/.a5c/processes/`.
