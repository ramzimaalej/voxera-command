...
1. Author the subagent pair in `voxera-crm/.claude/agents/`: `<lens>-designer.md` and `<lens>-reviewer.md` (follow the existing six as templates — designer produces a markdown design artifact; reviewer returns `{ verdict, score, findings }` and defaults to skepticism).
