---
title: Outbox Implementation — Postgres-only Transactional Outbox
version: 1
status: active
updated: 2026-05-31
owner: you
...
# Outbox Implementation — Postgres-only Transactional Outbox

> **Editorial note**: This doc is the **implementation-level spec** for the outbox referenced architecturally in [ADR-0003](../../decisions/ADR-0003-two-log-audit-model.md) (two-log audit model) and described at the patterns level in [`cross-cutting-patterns.md`](./cross-cutting-patterns.md) §Part 2.1. The decision to adopt this specific implementation (Postgres-only, 32 partitions, strict-ordered vs free-order split, advisory-lock partition ownership) is recorded as [ADR-0011](../../decisions/ADR-0011-postgres-outbox-implementation.md). Where this doc and ADR-0003 disagree on intent, ADR-0003 wins; where they disagree on mechanism, this doc wins.
...
2. **One slow downstream stalls everything.** A rate-limited Slack call, a brownout from a SaaS API, a multi-second ML extraction — naively, they queue behind the realtime UI emit and block it.
...
BEGIN;
  ...
  INSERT INTO outbox (id, sequence, partition, type, payload, ...)
    VALUES (..., 'LeadStageChanged', '{"from":"Quoted","to":"Won"}', ...);
COMMIT;
```
...
await outbox.emit(tx, event);    // tx is the in-flight Prisma/Knex transaction
```

The `emit` call is a `tx.outbox.create({ data })` against the *same* tx as the primary write. The pattern's atomicity guarantee depends on this — no async fire-and-forget, no separate connection, no "I'll write the event later".
...
A single Node process can host all categories (lightweight) or split them across processes (HA / per-category capacity scaling). The choice is operational, not architectural — the consumer code is identical.
...
- **Projection ordering.** Materialized views and analytics tables that UPSERT-on-conflict need events applied in sequence order. Applying "Stage = Won" before "Stage = Quoted" silently corrupts the projection.
...
The cursor advances **before** the handlers run. A Slack 503, an OpenSearch timeout, an SQS region brownout — none of them stall the queue. Dedup is per `(group, handler, sequence)` in a separate table (§2.7). If a process crashes between handler success and the dedup-row insert, the next sweep re-runs the handler; idempotent handlers (anything that takes a stable id as a key) tolerate the double-run.

Use free-order for: SocketIO emits, search-index upserts, Slack/SQS/queue sends, external HTTP APIs — anything that's (a) idempotent at the side-effect layer, and (b) where ordering doesn't matter or is enforced at the side-effect destination.
...
- Realtime consumers (SocketIO, cache invalidation): 2s. Handlers should finish in <200ms p99; 10× headroom.
- Search-index, analytics, queue sends: 5s. Allows for index-cluster recovery or AWS round-trip jitter.
- Slack, cross-network HTTP: 10s. One rate-limit backoff cycle.
- ML/PDF extraction Lambdas: 30s. Matches the Lambda's own deadline.
...
| **Search indexation** | Search engine upserts | Free-order | OpenSearch / Elasticsearch / Algolia |
| **Notifications** | Chat/Slack/email | Free-order | Slack Web API, transactional email |
| **Queue fan-out** | AWS SQS / Pub-Sub / Kafka producer | Free-order | Cross-service messaging |
| **Heavy compute** | Lambda / ML pipeline triggers | Free-order, long timeout | PDF parsing, embeddings, OCR |

Each group has its own cursor, its own retry sweep, and its own advisory-lock range. A slow Slack downstream **cannot** stall the real-time fan-out — they run on independent partition leases.
...
The table is range-partitioned by `sequence`. Once every consumer group's cursor has passed a child partition's max sequence, that partition can detach to cold storage (S3, Glacier) — the active footprint stays at 30–60 days.
...
5. **Per-group cursors and per-handler dedup tables** isolate slow downstreams. A Slack 503 doesn't stall the SocketIO emit; an OpenSearch outage doesn't stall the cache invalidation. The architecture is naturally rate-limit-resilient.
...
**What runs out before Postgres does** is the external integrations themselves: Slack's rate limit, the search cluster's index pressure, the Lambda concurrency cap. That's exactly the right failure mode — and it's exactly why every external integration lives in its own consumer group, fronted by per-handler dedup and per-group retry sweeps.
...
2. **Command service** opens a Prisma/Knex transaction.
...
9. **Free-order consumers** claim in parallel; cursor advances first, handlers fire post-commit:
   ...
   - Heavy-compute consumer enqueues a Lambda / ML job.

User-visible latency:
- Real-time UI refresh: <1s.
- Chat notification: 1–3s (Slack API + rate-limit jitter).
...
## Changelog
- 2026-05-31 v1: imported from external design doc and adapted to Voxera architecture. Editorial note added linking to ADR-0003 (the architecture-level outbox decision this implements) and ADR-0011 (the implementation-choice decision capturing the specific design — Postgres-only, 32 partitions, strict-ordered vs free-order split, advisory locks).
