...
- `design-review.js` — engineering design gate: per-discipline design → adversarial-review → converge loops producing a design dossier. Ships the **DDD → ERD → Prisma → GraphQL → Webhook → AI-agent → Analytics → OO → TDD** lenses (`*-designer`/`*-reviewer` subagents in `voxera-crm/.claude/subagents/`). Runs standalone, and gates `implement-feature` before planning. Extend via `LENS_REGISTRY`.
